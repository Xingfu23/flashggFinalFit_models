...
#        self.modelBuilder.factory_('expr::muVc_ZZ("@2/(@0**4+@1**4*{sigma3_VBF_ZZ}/{sigma1_VBF_ZZ})", a1,a3,muV)'.format(**xsecs))
    ...
    def getYieldScale(self,bin,process):
        ...
        if process in ["wh_%sf05ph0_%s_hgg"%(self.altSignal,y) for y in years]:
            scale = 'intCoupling_WH'
        if process in ["zh_%sf05_%s_hgg"%(self.altSignal,y) for y in years]: # for VHlep naming
            scale = 'intCoupling_ZH'
        if process in ["wh_%sf05_%s_hgg"%(self.altSignal,y) for y in years]: # for VHlep naming
            scale = 'intCoupling_WH'
